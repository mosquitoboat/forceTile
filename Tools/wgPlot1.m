...
function [hE,hV]=wgPlot1(adjMat,coord,varargin)
...
% axesArea(h,[6 7 5 5]);
plotParm={'markerSize',1,'lineWidth',1.5,'marker','.','MarkerEdgeColor',[1,0.5,0.2]};
siz=size(adjMat);
vrtxSiz=100;
edgeMap=cool;
vrtxMap=summer;
...
for kk=1:2:nVarArgin
    ...
    case 'colormaprange'
           minEWt = min(varargin{kk+1});
           maxEWt = max(varargin{kk+1});
    otherwise
...
if eWeighted
  ...
  for kk=1:neColor
    ...
    hE=[hE,plot(x(:),y(:),'linewidth',1,'color',edgeMap(kk,:),plotParm{:})];
%     Draw legend bar
